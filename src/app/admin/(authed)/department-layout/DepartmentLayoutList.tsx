...
import {
  deleteDepartmentLayoutAction,
  reorderDepartmentLayoutAction,
} from '@/lib/admin-actions/department-layout';
import { useAdminListItems } from '@/lib/hooks/useAdminListItems';
import { useConfirm } from '@/components/admin/ConfirmDialogProvider';

export default function DepartmentLayoutList({ items: initialItems }: { items: DepartmentLayout[] }) {
  ...
  async function handleDelete(id: string, title: string) {
    const ok = await confirm({
      title: 'Delete department layout?',
    ...
    if (res.ok) {
      removeById(id);
      toast.success('Department layout deleted');
      router.refresh();
    } else {
  ...
  if (items.length === 0) {
    return (
      <div className="text-center py-12 bg-white border border-dashed border-gray-300 rounded-lg">
        <p className="text-gray-500 text-sm">No department layout entries yet.</p>
        <Link href="/admin/department-layout/new" className="text-accent hover:underline font-medium text-sm mt-2 inline-block">
  ...
  return (
    <SortableList
      ...
      getId={(p) => p.id}
      onReorder={async (ids) => {
        const res = await reorderDepartmentLayoutAction(ids);
      ...
      renderItem={(p) => (
        <div className="flex items-center justify-between gap-4 min-w-0">
          ...
          <div className="flex items-center gap-1 shrink-0">
            <Link href={`/admin/department-layout/${p.id}`} aria-label="Edit department layout"
            ...
            <button type="button" onClick={() => handleDelete(p.id, p.shortTitle)} aria-label="Delete department layout"
