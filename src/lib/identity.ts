...
export const getDepartmentIdentity = cache(async () => {
  ...
  if (!dept) {
    throw new Error(
      'DepartmentIdentity row missing (id="singleton"). Restore the required CMS data.',
    );
  }
  return dept;
});
...
export const getUniversityIdentity = cache(async () => {
  ...
  if (!uni) {
    throw new Error(
      'UniversityIdentity row missing (id="singleton"). Restore the required CMS data.',
    );
  }
  return uni;
});
...
export const getAboutInnovationHub = cache(async () => {
  return prisma.aboutInnovationHub.findUnique({ where: { id: 'singleton' } });
});
...
//    file-based reads everywhere except manual bootstrap/recovery.
// ─────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────
//  Admission lead popup — homepage-only timed lead-capture modal.
// ─────────────────────────────────────────────────────────────────
